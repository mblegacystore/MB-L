...
async function onIncompletePaymentFound(payment) {
    ...
    try {
        let res = await fetch("/api/cuci.js", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId: payment.identifier }) });
        let data = await res.json();
        pendingIncompleteCount--;
        updateStatus(data.success ? "Selesai" : "Dibersihkan");
        tryEnablePaymentButtons();
        return { status: data.success ? "COMPLETED" : "CANCELLED" };
    } catch (e) {
        pendingIncompleteCount--;
    ...
    try {
        ...
        if (payments && payments.length > 0) {
            ...
            for (let p of payments) {
                await fetch("/api/cuci.js", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId: p.identifier }) });
            }
            updateStatus("Sedia untuk pembayaran baru.");
        }
    } catch (e) { console.error("bersihkanSebelumBayar error:", e.message); }
}

// ========== U2A: BELI PRODUK ==========
async function buyProduct(key, amount) {
    if (!currentUser) { updateStatus("Sila login dahulu."); return; }
    if (key === "echelon" && localStorage.getItem('mb-legacy-bought-echelon') === 'true') { showEchelonReport(); return; }
    if (key === "command" && localStorage.getItem('mb-legacy-bought-command') === 'true') { showLockedContent('command'); return; }
    ...
    Pi.createPayment(
        { amount: parseFloat(total), memo: "MBL Store", metadata: { product: key } },
        {
            ...
            onReadyForServerApproval: id => fetch("/api/bayar-sah.js", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId: id }) }),
            onReadyForServerCompletion: (id, txid) => {
                fetch("/api/bayar-selesai.js", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId: id, txid: txid }) })
                    .then(() => {
                        updateStatus("Berjaya!");
                        if (key === "echelon") { localStorage.setItem('mb-legacy-bought-echelon', 'true'); currentUser.boughtEchelon = true; showEchelonReport(); }
                        if (key === "command") { localStorage.setItem('mb-legacy-bought-command', 'true'); currentUser.boughtCommand = true; showLockedContent("command"); }
                    })
                    .catch(async () => {
            ...
            onCancel: () => updateStatus("Dibatalkan"),
            onError: e => updateStatus("Ralat: " + e.message)
        }
    );
}
...
async function requestPayout() {
    ...
    if (now - lastPayoutTime < PAYOUT_COOLDOWN) {
        updateStatus("Sila tunggu " + Math.ceil((PAYOUT_COOLDOWN - (now - lastPayoutTime)) / 60000) + " minit lagi.");
        return;
    }
    ...
    try {
        const response = await fetch("/api/bayar-keluar.js", {
            ...
            body: JSON.stringify({ uid: currentUser.uid, amount: 0.1, accessToken: currentUser.accessToken, metadata: { source: "claim_reward", timestamp: Date.now() } })
        });
        ...
        if (result.success) {
            ...
            if (typeof showSuccessPopup === 'function') showSuccessPopup("✅ REWARD RECEIVED!", "0.1 Test-Pi sent to your wallet.", "OK");
        } else {
...
async function doLogin(isSilent = false) {
    ...
    try {
        ...
        currentUser = { uid: auth.user.uid, username: auth.user.username, wallet_address: auth.user.wallet_address || "", accessToken: auth.accessToken };
        ...
        if (!isSilent) updateStatus("Login gagal: " + e.message);
        else { document.getElementById("btn-login").style.display = "block"; updateStatus("Sila login semula"); }
    }
}
...
async function restoreSession() {
    ...
    try {
        ...
        currentUser = { uid: userData.uid, username: userData.username, wallet_address: userData.wallet_address || "", accessToken: userData.accessToken };
        updateStatus("Welcome back: " + currentUser.username);
        document.getElementById("btn-login").style.display = "none";
        tryEnablePaymentButtons();
        await doLogin(true);
    } catch (e) { console.error("Gagal memulihkan sesi:", e); }
}
